...
# upstream/ (vendored reference) is excluded, so the check behaves
# identically locally and in CI.
GOFMT_SRC := cmd internal

.PHONY: build vet tool-check gofmt-check lint lint-golangci qa install clean
...
# qa bundles the lint gate and the cross-compile matrix that catches the
# platform path/embed classes before CI does.
qa: lint
