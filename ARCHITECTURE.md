...
| **Smart dependency solver** | **Implemented (M4).** Persistent versioned cache (`os.UserCacheDir()/xkvm/fetch`, e.g. `~/Library/Caches/xkvm/fetch`) keyed by `Package+Version` as `<id>__<ver>.deb`; a cached copy whose sha256 matches the index is reused instead of re-downloaded (a sha mismatch re-downloads; the same dependency needed twice — same run or any run inside the 7-day TTL — is served from disk). Lazy prune: `Resolve` drops entries older than 7 days (mtime) — but only when `cacheDir` IS the standard persistent cache; a caller-supplied folder (the TUI's download destination) is the user's own files and is never auto-deleted (`isDefaultCacheDir`). Last-resort fallback: when explicit `--apt-source` repos and the Canister index both miss, the built-in default repo sweep (`repos.go`, the active ios-repo-updates-era repo list) is consulted; the whole sweep shares one `fallbackSweepTimeout` (30s) deadline so a dead id can't stall on hanging indexes. `defaultRepos` is a var so tests point it at httptest servers (the suite pins it to nil in `TestMain` — no test ever sweeps the real list) |
...
```
xKVM/
├── go.mod                        # module github.com/blessed0x/xkvm-ios-injector (rename on publish)
├── cmd/xkvm/
...
only). The advisory is emitted byte-identical to upstream's echo -e output
(patch.sh lines 341-347): a `=> <path>` line (non-Mach-O files only) then a
...
(exactly upstream's find-loop exclusion). Output is byte-identical to upstream's
banner: `=> <path>` for non-Mach-O files, then a
`*****fixed-paths-warnning*****` block with each surviving string on its
own line (patch.sh lines 341-347, spelling included), no log prefix and no
summary line. Informational, never fatal.
...
- `go install github.com/blessed0x/xkvm-ios-injector@latest` (or `./cmd/xkvm` build) → global binary.
...
1. **Module path** — placeholder `github.com/blessed0x/xkvm-ios-injector`; set real org on publish.
